...
$(document).ready(function () {
  ...
  // --- DataTable ---
  const table = $("#privacy_table").DataTable({
    ajax: {
      url: "/privacyPolicy/fetch",
      type: "GET",
      ...
      { data: "main_heading" },
      {
        ...
        render: (sections) => {
          ...
          const preview = sections
            .slice(0, 2)
            .map((s) => `<b>${s.heading}</b>: ${s.content.substring(0, 60)}...`)
            .join("<br>");
  ...
  $("#createPolicyForm").on("submit", function (e) {
    ...
    // convert to single section array
    const sections = [{ heading: "Section 1", content: sectionsRaw }];

    $.ajax({
      url: "/privacyPolicy/create",
      method: "POST",
      headers: { Authorization: "Bearer " + token },
      ...
      error: (xhr) =>
        toastr.error(xhr.responseJSON?.message || "Error creating policy."),
    });
  });
  ...
  $(document).on("click", ".edit-btn", async function () {
    ...
    try {
      const res = await fetch(`/privacyPolicy/fetch/${id}`, {
        headers: { Authorization: "Bearer " + token },
      });
      const data = await res.json();

      if (!data.success) return toastr.error("Failed to fetch policy.");

      const item = data.data;
      $("#editPolicyId").val(item.policy_id);
      ...
      toastr.error("Error loading Privacy Policy.");
    }
  });

  // --- UPDATE ---
  $("#editPolicyForm").on("submit", async function (e) {
    e.preventDefault();
    const id = $("#editPolicyId").val();
    ...
    Array.from(tempDiv.children).forEach((el) => {
      if (el.tagName === "H5") {
        if (current) {
          current.content = current.content.replace(/<p><br><\/p>/g, "").trim();
    ...
    $.ajax({
      ...
      error: (xhr) =>
        toastr.error(xhr.responseJSON?.message || "Error updating policy."),
    });
  });
  ...
  $(document).on("click", ".delete-btn", function () {
    ...
    $.ajax({
      ...
      error: (xhr) =>
        toastr.error(xhr.responseJSON?.message || "Error deleting policy."),
    });
  });
});
